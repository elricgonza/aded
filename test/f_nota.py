...
def get_cuotas_por_curso(cur_id):
    ...
    cur = cnx.cursor()
    try:
        cur.execute(f"select nro_cuota, cuota from costo where cur_id = {cur_id}")
        cuotas = cur.fetchall()
    ...
    try:
        ...
        for inscrito in inscritos:
            ...
            for cuota in cuotas:
                ...
                pagos.append((pag_id, ins_id, nro_cuota, cuota, pagado, metodo_pago, \
                                  fecha_pago, referencia_pago, obs, creado, act, usu_id))
                pag_id += 1

                # ── 4. Insertar en lotes ───────────────────────────────────────────────
                insert_sql = """
                    INSERT INTO pago (id, ins_id, nro_cuota, cuota, pagado, metodo_pago, \
                        fecha_pago, referencia_pago, obs, creado, act, usu_id)
                    VALUES (%s, %s, %s, %s,  %s, %s, %s, %s,  %s, %s, %s, %s)
                    ON CONFLICT DO NOTHING;
                """
        cur.executemany(insert_sql, pagos)
        conn.commit()
        ...
        print(f"✔  Pagos insertados         : {len(pagos)}")
